		...
		while not self.done:
			for robot in self.robotList:
				if robot and self.go:
					...
					#3D MAZE:
					# if(robot.get_proximity(1) > 10): #avoid right wall
					# 	robot.set_wheel(1, 20)
					# 	robot.set_wheel(0, 10)

					if(robot.get_proximity(0) > 40): #turn left
						robot.set_wheel(0, -30)
						robot.set_wheel(1, 60)

					elif(robot.get_proximity(1) < 30): #stay close to right wall
						robot.set_wheel(0, 30)
						robot.set_wheel(1, 5)
					...
					print "left " + str(robot.get_proximity(0))
					print "right " + str(robot.get_proximity(1))
