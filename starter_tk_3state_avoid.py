...
class RobotBehavior(object):
	...
	def spawn_threads(self):
		...
		#add all the states:
		sm.add_state('S_moving_forward', 'object_left', self.turning_right, 'S_turning_left')
		sm.add_state('S_moving_forward', 'object_right', self.turning_left, 'S_turning_right')
		sm.add_state('S_turning_left', 'object_right', self.turning_left, 'S_turning_right')
		...
		sm.add_state('S_turning_right', 'object_left', self.turning_right, 'S_turning_left')
	...
	def event_watcher(self, q):
		while not self.done:
			if self.robot_list and self.go:
				...
				#print(prox_l,prox_r)
				if(self.robot.get_floor(0)<50 or self.robot.get_floor(1)<50):
					self.robot.set_wheel(1,0)
					self.robot.set_wheel(0,0) 				
					self.robot.set_musical_note(40)
					self.robot.set_led(0, 2)
					...
					q.put(alert_event)
					print("object left")
				elif(prox_r > prox_l): #obstacle on the right
