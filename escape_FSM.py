        ...
        while not self.quit:
            for robot in self.robot_list:
                ...
                if(event.type=="obstacle"):
                    print("obstacle")
                    if(event.data[0]>event.data[1]): #if obstacle is on the left side
                        robot.set_wheel(0,30) #turn left
                        robot.set_wheel(1,-10)   
                    elif(event.data[0]<event.data[1]): #if obstacle is on the right side
                        robot.set_wheel(1,30) #turn right
                        robot.set_wheel(0,-10)   
                    #else: #alligned in front of obstacle
