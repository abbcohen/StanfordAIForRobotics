...
class MotionPlanner(object):
	...
	def worker(self):
		...
		search_methods = ['Dijkstra\'s', 'A*', 'BFS', 'DFS', 'Gradient Descent', 'Floyd', 'djk']
		while True:
			search = raw_input('Enter the desired search method out of: Dijkstra\'s, A*, BFS, DFS, Gradient Descent\n')
		...
		if search == search_methods[0]: 
			...
			if len(path) > 0:
				for node in path:
					px = myGraph.nodes[node].data[0]
					py = myGraph.nodes[node].data[1]
					vWorld.canvas.create_oval(canvas_width+px-6, canvas_height-py-6, canvas_width+px+6, canvas_height-py+6, outline = "red", fill="red")
					#vWorld.canvas.create_text(canvas_width+px, canvas_height-py, text=node)
			else:
				print "failed to find path"
			...
			pathToGoal = myGraph.Gradient_Descent(vWorld, [200, 0], [self.vWorld.goal_x, self.vWorld.goal_y])
			for node in pathToGoal:
				vWorld.canvas.create_oval(canvas_width+node[0]-6, canvas_height-node[1]-6, canvas_width+node[0]+6, canvas_height-node[1]+6, outline = "red", fill="red")
			if len(pathToGoal) > 0:
				pass
				#print 'path: ', pathToGoal
			else:
				print 'failed to find path'
		elif search == search_methods[5]:
			pathToGoal = myGraph.Floyd()
			if len(pathToGoal) > 0:
				print pathToGoal
				for node in pathToGoal:
					px = node.data[0]
				...
				print 'failed to find path'
		elif search == search_methods[6]:
			myGraph.djk()
			node = myGraph.nodes['g']
			if not node.back_pointer == False:
				while not node.back_pointer == False:
					print 'looped', node.name
					px = node.data[0]
					py = node.data[1]
					vWorld.canvas.create_oval(canvas_width+px-6, canvas_height-py-6, canvas_width+px+6, canvas_height-py+6, outline = "red", fill="red")
		...
		self.vWorld.times.append([search, tTime])
		for t in self.vWorld.times:
			print 'Time Taken for', str(t[0])+':', t[1]
	...
	def compute_free_cells(self, cell_list, c_obs_list):
		...
		for rect in c_obs_list:
			self.vWorld.canvas.create_rectangle(self.vWorld.canvas_width+rect[0], self.vWorld.canvas_height-rect[1], self.vWorld.canvas_width+rect[2], self.vWorld.canvas_height-rect[3], fill='red', width='1')
		#print '*******'
		#line = vLines[1]
		#print test_point
		...
		for line in vLines:
			#self.vWorld.canvas.create_line(self.vWorld.canvas_width+line[0], self.vWorld.canvas_height-line[1], self.vWorld.canvas_width+line[2], self.vWorld.canvas_height-line[3], fill='blue')
			#self.vWorld.canvas.create_oval(self.vWorld.canvas_width+line[0]-2, self.vWorld.canvas_height-line[1]-2, self.vWorld.canvas_width+line[0]+2, self.vWorld.canvas_height-line[1]+2, fill='green')
			#self.vWorld.canvas.create_oval(self.vWorld.canvas_width+line[2]-2, self.vWorld.canvas_height-line[3]-2, self.vWorld.canvas_width+line[2]+2, self.vWorld.canvas_height-line[3]+2, fill='red')
...
class GUI(object):
	...
	def initUI(self):
		...
		rCanvas  = tk.Canvas(self.gui_root, bg="light gray", width=canvas_width*2, height= canvas_height*2, bd = 0, highlightthickness = 0, relief = 'ridge')
...
class VirtualWorld(object):
	...
	def create_world(self):
		...
		canvas_width = 300
		canvas_height = 200
		self.vWorld.area = [-canvas_width,-canvas_height,canvas_width,canvas_height]
		
		val = None
		while(True):
			obs = raw_input('Input the number of desired obstacles\n')
			try:
				val = int(obs)
				break
			except ValueError:
		...
		while i < val:
			...
			#print x, y
			if x + 2 < canvas_width and y - 2 > -canvas_height:
				if 200 >= x-2-28 and 200 <= x+2+28 and 0 >= y-2-28 and 0 <= y+2+28:
					pass
				else:
					self.vWorld.add_obstacle([x, y-2, x+2, y])
					i += 1
				#print [x, y-2, x+2, y]
