...
class MotionPlanner(object):
    ...
    def worker(self):
        ...
        #     self.canvas.create_rectangle(cell[0], cell[1], cell[2], cell[3], fill="yellow")
    ...
    def compute_c_obstacles(self, vworld, d):
        ...
        for obstacle in vworld.map:
            vworld.cobs.append([min(obstacle[0], obstacle[2])-d, min(obstacle[1], obstacle[3])-d, max(obstacle[0], obstacle[2])+d, max(obstacle[1], obstacle[3])+d])

    def compute_free_cells(self, cell_list, c_obs_list):
        cells = []
        for cobs in self.vWorld.cobs:
            x1 = cobs[0]
            y1= cobs[1]
            x2= cobs[2]
            y2 = cobs[3]
            ...
            bottomRight=[x2,y2]
            #up:
            cells.append([topLeft[0], 0, topRight[0],topRight[1]])
            #down:
            # cells.append([bottomLeft[0], bottomLeft[1], bottomRight[0],self.vWorld.canvas_height*2])
        ...
        #             points[i+1] = temp
        # ylines.append(0)
        # ylines.append(self.canvas_height)
        # for i in range (0,len(points)-1):
        #     xlines.append(points[i][0], points[0][1])
            # ylines.append(points[i][1])
