...
class virtual_world:
    ...
    def in_collision(self, a, x, y):
        ...
        FR_y= self.canvas_height - self.vrobot.l * math.cos(pof) - y
        BR_y= self.canvas_height - self.vrobot.l * math.cos(tpof) - y
        BL_y= self.canvas_height - self.vrobot.l * math.cos(fpof) - y
        FL_y= self.canvas_height - self.vrobot.l * math.cos(spof) - y

        FR_x= self.canvas_width + self.vrobot.l * math.sin(pof) + x
        BR_x= self.canvas_width + self.vrobot.l * math.sin(tpof) + x
        BL_x= self.canvas_width + self.vrobot.l * math.sin(fpof) + x
        FL_x= self.canvas_width + self.vrobot.l * math.sin(spof) + x

        myCorners = [[FR_x,FR_y],[BR_x,BR_y],[BL_x,BL_y], [FL_x, FL_y]]
        FR = myCorners[0]
        BR = myCorners[1]
        BL = myCorners[2]
        FL = myCorners[3]
        ...
        for obs in self.map:
            #set coordinates of obstacle
            x1 = self.canvas_width+obs[0]
            y1 = self.canvas_height-obs[1]
            x2 = self.canvas_width+obs[2]
            y2 = self.canvas_height-obs[3]
            lilX=min(x1, x2)
            bigX=max(x1, x2)
            lilY=min(y1, y2)
            bigY=max(y1, y2)
            if (self.PointInHamster(FR, BR, BL, FL, x1, y1) or self.PointInHamster(FR, BR, BL, FL, x2, y1)\
             or self.PointInHamster(FR, BR, BL, FL, x1, y2) or self.PointInHamster(FR, BR, BL, FL, x2, y2)):
                return True
            if((FR_x>=lilX and FR_x<=bigX) and (FR_y>=lilY and FR_y<=bigY)):
                print "side" 
                return True
            if((BR_x>=lilX and BR_x<=bigX) and (BR_y>=lilY and BR_y<=bigY)):
                print "side" 
                return True
            if((BL_x>=lilX and BL_x<=bigX) and (BL_y>=lilY and BL_y<=bigY)):
                print "side" 
                return True
            if((FL_x>=lilX and FL_x<=bigX) and (FL_y>=lilY and FL_y<=bigY)):
                print "side" 
                return True
        return False


    def PointInHamster(self, FR, BR, BL, FL, x, y):
        x1 = FR[0]
        y1 = FR[1]
        x2 = BR[0]
        y2 = BR[1]
        ...
        a_total = (self.triangleArea(x1,y1,x2,y2,x3,y3)) + self.triangleArea(x1,y1,x4,y4,x3,y3)
        a1 = self.triangleArea(x1, y1, x2, y2, x, y)
        a2 = self.triangleArea(x4, y4, x1, y1, x, y)
        a3 = self.triangleArea(x2, y2, x3, y3, x, y)
        a4 = self.triangleArea(x3, y3, x4, y4, x, y)
        #print "total: " + str(a_total)
        #print "triangles: " + str(a1+a2+a3+a4)
        if(abs(a_total-a1-a2-a3-a4)<10):
            print "corner"
            return True
        else:
            return False
    def triangleArea(self, x1, y1, x2, y2, x3, y3):
        a = self.getDistance(x1, y1, x2, y2)
        b = self.getDistance(x2, y2, x3, y3)
        c = self.getDistance(x1, y1, x3, y3)
        sp = (a+b+c)/2
